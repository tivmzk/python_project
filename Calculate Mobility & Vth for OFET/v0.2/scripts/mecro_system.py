...
class action_file:
    ...
    def get_strs(self, arr):
        ...
        if not self.is_print_rt:
            text = arr[14] + "," + arr[13] + "," + arr[15] + ",," + \
                str(self.ori_vg) + "," + str(self.ab_id) + \
                "," + str(self.ab_ig) + "\n"
        else:
            text = arr[14] + "," + arr[13] + "," + arr[15] + ",," + str(self.ori_vg) + "," + str(
                self.ab_id) + "," + str(self.ab_ig) + "," + str(self.rt_id) + "\n"
    ...
    def calculate(self, W, L, C, point_count):
        ...
        for i in range(0, len(self.arr_x)):
            son.append((self.arr_x[i] - self.avg_x)
                       * (self.arr_y[i] - self.avg_y))
    ...
    def append_caculated_values(self, W, L, C):
        ...
        self.arr_values[0] = str(tmp).replace('\n', '') + ',,' + self.file_name + ',' + str(self.slope) + ','\
            + str(self.inter_x) + ',' + str(self.inter_y) + ',' + \
            str(self.mob) + ',' + str(W) + ',' + str(L) + ',' + str(C) + '\n'
...
class system:
    ...
    def start(self, folder_path, result_folder_path):
        try:
            ...
            for file_path in file_list_csv:
                file = action_file(folder_path + '/' + file_path,
                                   result_folder_path + '/' + file_path, file_path)
                file.start(self.get_L(), self.get_W(), self.get_C(
                ), self.get_FV(), self.get_point_count(), self.get_is_n_type())
    ...
    def create_result_list_file(self, result_list, result_folder_path):
        ...
        file.write(
            'file name,slope,intercept,Vth,mobility,Width,Length,Capacitance\n')
