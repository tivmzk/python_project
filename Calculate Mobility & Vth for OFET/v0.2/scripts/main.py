...
class Program:
    def __init__(self):
        ...
        self.btn_folder_path = tkinter.Button(
            self.window, text='작업 폴더 지정', command=self.search_dir_folder)
        self.btn_folder_path.grid(row=0, column=0)

        self.txt_folder_path = tkinter.StringVar()
        self.txt_folder_path.set('작업 폴더를 지정해 주세요')

        self.lb_folder_path = tkinter.Label(
            self.window, textvariable=self.txt_folder_path)
        self.lb_folder_path.grid(row=0, column=1)

        self.btn_result_path = tkinter.Button(
            self.window, text='결과 폴더 지정', command=self.search_dir_result)
        self.btn_result_path.grid(row=1, column=0)

        self.txt_result_path = tkinter.StringVar()
        self.txt_result_path.set('결과 폴더를 지정해 주세요')

        self.lb_result_path = tkinter.Label(
            self.window, textvariable=self.txt_result_path)
        self.lb_result_path.grid(row=1, column=1)

        self.btn_help = tkinter.Button(
            self.window, text='설명', command=self.help, width=10)
        self.btn_help.grid(row=0, column=2, columnspan=2)

        self.btn_execute = tkinter.Button(
            self.window, text='실행', command=self.execute, width=10)
        self.btn_execute.grid(row=1, column=2, columnspan=2)

        self.txt_progress = tkinter.StringVar()
        self.txt_progress.set('대기 중')

        self.lb_progress = tkinter.Label(
            self.window, textvariable=self.txt_progress)
        ...
        self.cb_setting_fv = tkinter.Checkbutton(
            self.window, var=self.cb_setting_fv_var)
    ...
    def search_dir_folder(self):
        self.folder_path = filedialog.askdirectory(
            parent=self.window, initialdir='/', title='Please select a directory')
        self.txt_folder_path.set(self.folder_path)

    def search_dir_result(self):
        self.result_path = filedialog.askdirectory(
            parent=self.window, initialdir='/', title='Please select a directory')
